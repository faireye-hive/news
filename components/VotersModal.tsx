...
export const VotersModal: React.FC<VotersModalProps> = ({ post, isOpen, onClose, tribeInfo }) => {
  ...
  const votersList = useMemo(() => {
    ...
    if (post.pending_token != null && post.pending_token > 0) {
      postCalculatedReward = isLegacy ? (post.pending_token / Math.pow(10, precision)) : post.pending_token;
    } else {
      ...
      if (totalPaid > 0) {
        postCalculatedReward = isLegacy ? (totalPaid / Math.pow(10, precision)) : totalPaid;
        ...
        if (reward > 0) {
           postCalculatedReward = isLegacy ? (reward / Math.pow(10, precision)) : reward;
        }
      }
    }
