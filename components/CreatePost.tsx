...
const SUGGESTED_TAGS = ['news', 'crypto'];
...
const CreatePost: React.FC = () => {
  ...
  const [declinePayout, setDeclinePayout] = useState(true);
      ...
      <div className="bg-card p-6 md:p-8 rounded-2xl border border-slate-700/50 shadow-xl relative overflow-hidden">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-bold text-white flex items-center gap-3">
             <Edit3 className="text-cent" /> {t('createPost.title')}
          </h2>
        ...
        {!isPreview ? (
          <div className="space-y-4">
            <div>
              <label className="block text-slate-400 text-sm font-medium mb-1">{t('createPost.titleLabel')}</label>
              <input 
                type="text"
                value={title}
            ...
            <div>
              <label className="block text-slate-400 text-sm font-medium mb-1 flex justify-between">
                {t('createPost.bodyLabel')}
                <span className="text-xs text-slate-500">{t('createPost.bodyHint')}</span>
              </label>
              <textarea 
                value={body}
            ...
            <div className="bg-slate-800/50 p-4 border border-slate-700/50 rounded-xl space-y-4">
              <div>
                ...
                <input 
                  ...
                  disabled={isPublishing}
                />
                <p className="text-xs text-slate-500 mt-2">{t('createPost.tagsHint')}</p>
              </div>

              <div className="border-t border-slate-700 pt-4 mt-2">
                <label className="text-slate-400 text-sm font-medium mb-3 flex items-center gap-2">
                   <Settings size={16} /> {t('createPost.advOptions')}
                </label>
                <label className="flex items-center gap-3 cursor-pointer group">
                  ...
                  />
                  <div>
                    <div className="text-slate-300 font-medium text-sm group-hover:text-white transition-colors">{t('createPost.declinePayout')}</div>
                    <div className="text-xs text-slate-500">{t('createPost.declineDesc')}</div>
                  </div>
                </label>
              </div>
            </div>
          ...
          <div className="bg-slate-900 rounded-xl border border-slate-700 p-6 min-h-[500px]">
             ...
             {body ? (
                ...
                <div className="text-center text-slate-500 py-20">{t('createPost.nothingPreview')}</div>
             )}
          </div>
        )}
