      ...
      <div className="bg-gradient-to-r from-slate-900 to-slate-800 rounded-2xl p-8 border border-slate-700/50 shadow-xl overflow-hidden relative flex flex-col md:flex-row items-center justify-between gap-6">
        ...
        <div className="flex items-center gap-6 relative z-10 w-full md:w-auto">
          ...
          <div>
            ...
            <p className="text-slate-400 mb-2">{t('profile.profileIn')} <span className="font-bold text-cent">{community}</span></p>
            {scotData && scotData.voting_power !== undefined && (
              <div className="inline-flex items-center gap-1.5 px-2 py-0.5 bg-slate-800 rounded border border-slate-700">
                 ...
                 <span className="text-xs font-medium text-slate-300">{(scotData.voting_power / 100).toFixed(2)}% {t('profile.powerIn')} {community}</span>
              </div>
            )}
          </div>
        </div>
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div className="bg-card p-6 rounded-2xl border border-slate-700/50 flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    <span className="text-slate-400 text-xs text-transform uppercase tracking-wider block mb-1">{t('profile.wallet.liquidBalance')}</span>
                    <div className="text-2xl font-mono text-white mb-2">
                      {balance ? parseFloat(balance.balance).toLocaleString() : '0'} <span className="text-sm text-cent">{community}</span>
                    </div>
                    {user === currentProfile && (
                       <div className="flex gap-2">
                          <button onClick={() => setWalletActionType('transfer')} className="text-xs bg-slate-800 hover:bg-slate-700 text-white px-2 py-1 rounded">{t('profile.wallet.send')}</button>
                          <button onClick={() => setWalletActionType('stake')} className="text-xs bg-slate-800 hover:bg-slate-700 text-white px-2 py-1 rounded">{t('profile.wallet.powerUp')}</button>
                       </div>
                    )}
                  </div>
                </div>
                ...
                <div className="bg-card p-6 rounded-2xl border border-slate-700/50 flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    <span className="text-slate-400 text-xs text-transform uppercase tracking-wider block mb-1">{t('profile.wallet.staking')}</span>
                    <div className="text-2xl font-mono text-white mb-2">
                      {balance ? parseFloat(balance.stake).toLocaleString() : '0'} <span className="text-sm text-cent">{community}</span>
                    </div>
                    {user === currentProfile && (
                       <div className="flex gap-2">
                          <button onClick={() => setWalletActionType('unstake')} className="text-xs bg-slate-800 hover:bg-slate-700 text-white px-2 py-1 rounded">{t('profile.wallet.powerDown')}</button>
                          <button onClick={() => setWalletActionType('delegate')} className="text-xs bg-slate-800 hover:bg-slate-700 text-white px-2 py-1 rounded">{t('profile.wallet.delegate')}</button>
                       </div>
                    )}
                  </div>
                </div>
                ...
                <div className="bg-card p-6 rounded-2xl border border-slate-700/50 flex items-start gap-4">
                  ...
                  <div>
                    <span className="text-slate-400 text-xs text-transform uppercase tracking-wider block mb-1">{t('profile.wallet.pendingCuration')}</span>
                ...
                <div className="bg-card p-6 rounded-2xl border border-slate-700/50 flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    <span className="text-slate-400 text-xs text-transform uppercase tracking-wider block mb-1">{t('profile.wallet.delegationsOut')}</span>
                    <div className="text-2xl font-mono text-white mb-2">
                    ...
                    {user === currentProfile && parseFloat(balance?.delegationsOut || '0') > 0 && (
                       <div className="flex gap-2">
                          <button onClick={() => setWalletActionType('undelegate')} className="text-xs bg-slate-800 hover:bg-slate-700 text-white px-2 py-1 rounded">{t('profile.wallet.removeDelegation')}</button>
                       </div>
                    )}
                  </div>
                </div>
                ...
                <div className="bg-card p-6 rounded-2xl border border-slate-700/50 flex items-start gap-4">
                  ...
                  <div>
                    <span className="text-slate-400 text-xs text-transform uppercase tracking-wider block mb-1">{t('profile.wallet.delegationsIn')}</span>
                    <div className="text-2xl font-mono text-white">
             ...
             {walletActionType && (
                <div className="bg-slate-900 border border-slate-700 rounded-xl p-6 relative">
                   ...
                   <div className="space-y-4 max-w-md">
                      ...
                      <div>
                         <label className="block text-xs font-medium text-slate-400 mb-1">
                            {t('profile.wallet.quantity')} ({community})
                         </label>
                               ...
                               <div className="flex items-center gap-4">
                                  ...
                                  <div>
                                     ...
                                     <div className="text-xs text-slate-400">
                                        ...
                                        {item.from && item.from !== currentProfile && !item.from.includes('contract_') && ` • {t('profile.history.from')} ${item.from}`}
                                        {item.to && item.to !== currentProfile && !item.to.includes('contract_') && ` • {t('profile.history.to')} ${item.to}`}
                                        {item.authorperm && ` • {t('profile.history.post')} ${item.authorperm.split('/')[1] || item.authorperm}`}
                                        {item.memo && ` • {t('profile.history.memo')} ${item.memo}`}
                                     </div>
                                  </div>
                               </div>
                    ...
                    <div className="flex-1 flex flex-col min-w-0">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        {isComment && (
                           <>
                             <span className="text-slate-500 text-xs shrink-0">&bull;</span>
                             <span className="text-slate-500 text-xs truncate">{t('profile.inReplyTo')} @{post.parent_author}</span>
                           </>
                        )}
                      </div>
